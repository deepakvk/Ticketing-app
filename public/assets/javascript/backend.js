...
$(document).on('click','.delete-guest', function (e) {
  e.preventDefault();
  var clickedEl = $(this);
  var token, table, url, id, trans_id, span1_val, new1_val, span2_val, new2_val, upd_val;
  url   = clickedEl.attr('data-href');
  id    = clickedEl.attr('data-gstid');
  trans_id = clickedEl.attr('data-trans-id');
  table = "table_"+trans_id;
  span1_val = $('#cnt_'+trans_id).text();
  span2_val = $('#total_'+trans_id).text();

  if(confirm("Are you sure want to Delete the guest?")){
    ...
    $.ajax({
        ...
        success : function(resp){
           if(resp.status == 'success'){
				...
				upd_val = parseInt(span1_val)-1;            
				$('#cnt_'+trans_id).text(upd_val);						 
				$("#guestform_"+trans_id).find("#alert_"+trans_id).hide();
				$("#guestform_"+trans_id).find(".add_"+trans_id).show();
				$("#guestform_"+trans_id).find('.CreateGuest').show();				
				$("#guestform_"+trans_id).find(".guestcheckth_"+trans_id).show();
				$("#guestform_"+trans_id).find(".guestchecktd_"+trans_id).show();
				 if(upd_val > 0){
				$("#guestform_"+trans_id).find(".copy_"+trans_id).show();
				$("#guestform_"+trans_id).find('#replicateGuest_'+trans_id).show();
				}
				else{
					$("#guestform_"+trans_id).find(".copy_"+trans_id).hide();
					$("#guestform_"+trans_id).find('#replicateGuest_'+trans_id).hide();
					$('#'+table).find('tbody').append($('<tr>').append($('<td>').text('There are no guests added.')));
				}
           }		   
         }	 
        });		
  ...
  $('.replicate').on('click', function (e) {
    e.preventDefault();
    var clickedEl = $(this);
    var token, url, table, row, td, trans_id, span1_val, new1_val, span2_val, new2_val;
    token = $('input[name=_token]').val();
    url   = clickedEl.attr('data-href');
    table = clickedEl.attr('data-table');
    td    = clickedEl.attr('data-td');
    trans_id = clickedEl.attr('data-trans-id');
    span1_val = $('#cnt_'+trans_id).text();
    span2_val = $('#total_'+trans_id).text();
    
    if($("#guestform_"+trans_id+" input[type='radio']:checked").val()){
	
      var id = $("#guestform_"+trans_id+" input[type='radio']:checked").val();
       ...
       $.ajax({
           ...
           success : function(resp){
             if(resp.status == 'success'){
               ...
               var num = parseInt( $("#guestform_"+trans_id+" input[type='radio']:checked").closest("tr").prop("id").match(/\d+/g), 10 ) +1;
				
               $tr = $("#guestform_"+trans_id+" input[type='radio']:checked").closest("tr").clone().prop('id', 'klon'+num ).prop('class', 'klon_'+resp.guest_id);
               $tr.insertAfter($("#guestform_"+trans_id+" input[type='radio']:checked").closest("tr"));
			   
               $("#guestform_"+trans_id).find('tr[id^="klon"]:last').find('td:nth-child(1)').prop('id', resp.guest_id);
			   
               $('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(2)').text(resp.guest_id);

               $("#guestform_"+trans_id).find('tr[id^="klon"]:last').find('.delete-guest').attr('data-gstid', resp.guest_id);
               var route = $("#guestform_"+trans_id).find('tr[id^="klon"]:last').find('.delete-guest').attr('data-baseurl');
               $("#guestform_"+trans_id).find('tr[id^="klon"]:last').find('.delete-guest').attr('data-href', route+'/guests/'+resp.guest_id+'/delete');
               $("#guestform_"+trans_id).find('tr[id^="klon"]:last').find('.EditAllocation').attr('data-href', route+'/guests/'+resp.guest_id+'/'+trans_id+'/edit')

               
               var upd_val = parseInt(span1_val)+1;
               var copy_hide = parseInt(span2_val)-parseInt(upd_val);
               $('#cnt_'+trans_id).text(upd_val);
			   
               if(copy_hide==0){
					$('#replicateGuest_'+trans_id).hide();
                    $('#CreateGuest_'+trans_id).hide();                   
                    $(".guestcheckth_"+trans_id).hide();
                    $(".guestchecktd_"+trans_id).hide();
					$("#alert_"+trans_id).show();
               }            
             }
           }
       });
      ...
      success: function (data, statusText, xhr, $form) {
          ...
          switch (data.status) {
              case 'success':
					...
					/**** Create Guest modal Popup *****/
					if($form.hasClass('create_guest')) {
						
						var alloc_id = $form.find('input[name=alloc_id]').val();
						var table = "table_"+alloc_id;
						
						if($("#guestform_"+alloc_id+" input[type='radio']:last").length) {
							var num = parseInt( $("#guestform_"+alloc_id+" input[type='radio']:last").closest("tr").prop("id").match(/\d+/g), 10 ) +1;	

							$tr = $("#guestform_"+alloc_id+" input[type='radio']:last").closest("tr").clone().prop('id', 'klon'+num ).prop('class', 'klon_'+data.id);
							$tr.insertAfter($("#guestform_"+alloc_id+" input[type='radio']:last").closest("tr"));
							
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(1)').prop('id', data.id);
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(2)').text(data.id);
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(3)').text(data.guestName);
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(4)').text(data.guestCompany);
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(5)').text(data.jobTitle);
							$('#'+table).find('tr[id^="klon"]:last').find('td:nth-child(6)').text(data.specRequest);
							var route = $("#guestform_"+alloc_id).find('tr[id^="klon"]:last').find('.delete-guest').attr('data-baseurl');
							$('#'+table).find('tr[id^="klon"]:last').find('.delete-guest').attr('data-href', route+'/guests/'+data.id+'/delete');
							$('#'+table).find('tr[id^="klon"]:last').find('.EditAllocation').attr('data-href', route+'/guests/'+data.id+'/'+alloc_id+'/edit');
							var span1_val = $('#cnt_'+alloc_id).text();
							var span2_val = $('#total_'+alloc_id).text();
							var upd_val = parseInt(span1_val)+1;
							var copy_hide = parseInt(span2_val)-parseInt(upd_val);
							$('#cnt_'+alloc_id).text(upd_val);
						   
							if(copy_hide==0){
								$('.copy_'+alloc_id).hide();
								$('#replicateGuest_'+alloc_id).hide();
								$('.add_'+alloc_id).hide();
								$('#CreateGuest_'+alloc_id).hide();                   
								$(".guestcheckth_"+alloc_id).hide();
								$(".guestchecktd_"+alloc_id).hide();
								$("#alert_"+alloc_id).show();
							}
						}
						else {
							$('#'+table).find('tbody').find('tr').remove();
							$('#'+table).find('tbody').append($('<tr id="klon1" class="klon_'+data.id+'">').append($('<td id="'+data.id+'" class="guestchecktd_'+alloc_id+'">').append($('<input name="guestcheck" type="radio" class="guestcheck" id="guestcheck_'+alloc_id+'" value="'+data.id+'" />'))).append($('<td>').text(data.id)).append($('<td>').text(data.guestName)).append($('<td>').text(data.guestCompany)).append($('<td>').text(data.jobTitle)).append($('<td>').text(data.specRequest)).append($('<td>').append($('<a data-modal-id="EditAllocation" href="#" data-href="http://three-tickets/guests/'+data.id+'/'+alloc_id+'/edit" class="loadModal btn btn-xs btn-primary EditAllocation"> Edit</a>&nbsp;<button class="btn btn-xs btn-danger delete-guest" data-trans-id="'+alloc_id+'" data-gstid="'+data.id+'" data-href="'+location.protocol + "//" + location.host+'/guests/'+data.id+'/delete" data-baseurl="'+location.protocol + "//" + location.host+'"> Delete</button>'))));
							$('.copy_'+alloc_id).show();
							$('#replicateGuest_'+alloc_id).show();
							$('.rem_'+alloc_id).show();
							$('#cnt_'+alloc_id).text(1);
							
						}
					}
					/*** Edit Guest modal popup ***/
					if($form.hasClass('edit_guest')) { 
						var alloc_id = $form.find('input[name=alloc_id]').val();
						var guest_id = $form.find('input[name=guest_id]').val();
						var table = "table_"+alloc_id;
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(1)').prop('id', data.id);
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(2)').text(data.id);
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(3)').text(data.guestName);
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(4)').text(data.guestCompany);
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(5)').text(data.jobTitle);
						$('#'+table).find('tr[class="klon_'+guest_id+'"]').find('td:nth-child(6)').text(data.specRequest);
					}
                  var $submitButton = $form.find('input[type=submit]');
                  toggleSubmitDisabled($submitButton);
